...
// playing Mario Golf: Super Rush right now. Report active PIDs to nextendo-account every 30s so it keeps
...
func startPresenceReporter() {
	...
	go func() {
		for {
			...
			body, err := json.Marshal(map[string]any{"appId": golfAppID, "status": presenceStatus, "pids": active})
			if err != nil {
				continue
			}
			...
			if err != nil {
				fmt.Printf("[Golf Presence] batch report failed: %v\n", err)
				continue
			}
			if resp.StatusCode != http.StatusOK {
				respBody, _ := io.ReadAll(resp.Body)
				fmt.Printf("[Golf Presence] batch report rejected: HTTP %d: %s\n", resp.StatusCode, respBody)
			}
			resp.Body.Close()
		}
	}()
